...
import { TierLevels, buyableCryptoCurrencyList } from '@/utils/consts';
...
export async function POST(req: Request) {
  try {
    const { initData: telegramInitData, tier, walletAddress, cryptoCode, amount } = await req.json();

    if (!amount || !cryptoCode || !tier) {
      return NextResponse.json({ error: 'Amount, cryptoCode and tier are required' }, { status: 400 });
    }
    if (!walletAddress) {
    ...
    // Find the tier info
    const tierInfo = TierLevels.find(t => t.level === tier);
    if (!tierInfo) {
      return NextResponse.json({ error: 'Invalid Tier' }, { status: 400 });
    }
    
    // Find the crypto in our list to get the CoinGecko ID
    const cryptoAssetInfo = buyableCryptoCurrencyList.find(c => c.code === cryptoCode);
    if (!cryptoAssetInfo) {
      return NextResponse.json({ error: 'Invalid cryptoCode' }, { status: 403 });
    }
    ...
    let discount = tierInfo.discountPercent;
